...
## Install (other Windows PCs)

1. Open [Releases](https://github.com/thegalaxias-lab/codenotchforwindow/releases) and download the latest `codenotch-windows-x64.zip`.
2. Unzip anywhere (e.g. `Documents\Codenotch`).
3. Run `codenotch.exe`. Windows 10/11 already include WebView2 on most machines; if the window stays blank, install [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/).
4. Quit from the tray icon (right-click → Quit) before replacing the exe with a newer build.

To build from source instead: install Rust + MSVC Build Tools, then `cargo build --release` from this repo (or double-click `run.bat`).
