...
/// Bearer token the Grok Bot (Sand) DashboardService calls need. Same store as the Cursor ring;
/// never logged. `None` when Cursor is not signed in on this machine.
...
pub fn parse_summary(v: &serde_json::Value) -> (Vec<LimitWindow>, String) {
    ...
    let mut out = Vec::new();
    // Ring headline = the dashboard's combined number (0 is a reading too)
    ...
    // Hover breakdown: Auto/Cursor models vs named/other models. Always emit when the field is
    // present — 0 % is a real reading (the dashboard shows both bars even at zero).
