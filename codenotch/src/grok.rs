...
//!      ```
//!      The credits payload is the headline ring (SuperGrok / Grok Build weekly pool). Grok's own
//!      charge date is not in it — `billingPeriodEnd` is a calendar-month usage ledger, not a bill —
//!      so none is shown rather than guessed at.
//!   3. Grok Bot weekly limit (separate pool, separate reset): borrowed from Cursor's signed-in
//!      session via ConnectRPC `DashboardService/GetSandUsageStatus` on `api2.cursor.sh` — the same
//!      call the Grok Bot desktop app uses for Settings → Usage. Absent when Cursor is not signed in.
//!
...
/// Grok Bot weekly meter — same DashboardService method the Grok Bot app's Usage pane asks.
const BOT_USAGE_ENDPOINT: &str =
...
pub fn present() -> bool {
    auth_path().map(|p| p.is_file()).unwrap_or(false) || crate::cursor::access_token().is_some()
}
...
pub fn probe() -> String {
    ...
    let cli = if !p.is_file() {
        format!("Grok: {} not found (CLI not installed, or not signed in)", p.display())
    } else {
    ...
    let bot = if crate::cursor::access_token().is_some() {
        " | Bot via Cursor session"
    } else {
        " | Bot unavailable (Cursor not signed in)"
    };
    format!("{cli}{bot}")
}
...
/// `plan_label` overlays the headline when the credits payload has no product name (fresh weekly
/// period) — e.g. settings' `subscription_tier_display` ("SuperGrok Heavy").
pub fn parse_credits(v: &serde_json::Value, plan_label: Option<&str>) -> (Vec<LimitWindow>, String) {
    ...
    // Prefer the per-product bars when present: a SuperGrok account can expose Grok Build (and
    // others) separately. creditUsagePercent alone is the shared headline when products are absent.
        ...
        if weekly {
            out.push(LimitWindow {
                ...
                label: plan_label
                    .filter(|s| !s.is_empty())
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| "Weekly limit".into()),
                used: 0.0,
                resets_at,
                ..Default::default()
            });
...
/// Grok Bot desktop Settings → Usage (`GetSandUsageStatus`) → one weekly window, or None when the
/// account has no Bot entitlement / Cursor is signed out.
pub fn parse_bot_usage(v: &serde_json::Value) -> Option<LimitWindow> {
    // Absent fields mean no Bot meter on this account — not a fabricated 0 %.
    let used = pct(v.get("usagePercent"))?;
    let resets_at = iso_ms(v.get("nextResetTimestampUtc"));
    let label = v
        .get("grokPlanLabel")
        .and_then(|x| x.as_str())
        .filter(|s| !s.is_empty())
...
/// Best-effort plan name for a fresh weekly period that has no productUsage yet.
fn fetch_plan_label(token: &str) -> Option<String> {
...
/// Grok Bot weekly limit via Cursor's DashboardService. Failures are silent — Bot is additive.
fn fetch_bot_window() -> Option<LimitWindow> {
    let token = crate::cursor::access_token()?;
    let agent = ureq::AgentBuilder::new().timeout(Duration::from_secs(15)).build();
    let v: serde_json::Value = agent
        .post(BOT_USAGE_ENDPOINT)
        .set("Authorization", &format!("Bearer {token}"))
        .set("Content-Type", "application/json")
        .set("Connect-Protocol-Version", "1")
        .set("User-Agent", "Codenotch/1")
...
fn read_once(prev: &UsageSnapshot) -> UsageSnapshot {
    ...
    let bot = fetch_bot_window();
    let Some(creds) = read_credentials() else {
        // No Grok CLI session: still show Bot alone when Cursor is signed in.
        if let Some(w) = bot {
            snap.fetched_at = now_ms();
            snap.status = "ok".into();
            snap.windows = vec![w];
            snap.note = "Grok Bot · via Cursor".into();
            return snap;
        }
    ...
    match fetch_once(&creds.token) {
        Ok(v) => {
            let plan = fetch_plan_label(&creds.token);
            let (mut windows, note) = parse_credits(&v, plan.as_deref());
            if let Some(w) = bot {
                // One Bot bar; replace a stale copy if a previous poll already wrote one.
                windows.retain(|x| x.id != "bot");
                windows.push(w);
            }
            snap.fetched_at = now_ms();
            if windows.is_empty() {
        ...
        Err(FetchErr::NeedsAuth) => {
            if let Some(w) = bot {
                ...
                snap.status = "ok".into();
                snap.windows = vec![w];
                snap.note = "Grok CLI session rejected — Bot via Cursor".into();
            } else {
                snap.status = "needsAuth".into();
        ...
        Err(FetchErr::RateLimited) => {
            if let Some(w) = bot {
                ...
                snap.windows = windows;
                snap.fetched_at = now_ms();
                snap.status = if snap.windows.iter().any(|x| x.id == "credits") {
                    "stale"
                } else {
                    "ok"
                }
                .into();
                snap.note = "Grok is rate limiting; Bot reading is fresh".into();
            } else {
        ...
        Err(FetchErr::Other(msg)) => {
            if let Some(w) = bot {
                ...
                .into();
                snap.note = format!("{msg} · Bot via Cursor");
            } else {
...
mod tests {
    ...
    fn credits_percent_becomes_the_headline_window() {
        ...
        .unwrap();
        let (w, note) = parse_credits(&v, None);
    ...
    fn a_fresh_weekly_period_reads_as_zero_rather_than_unmetered() {
        ...
        .unwrap();
        let (w, note) = parse_credits(&v, Some("SuperGrok Heavy"));
        assert!(note.is_empty());
        assert_eq!(w.len(), 1);
        assert_eq!(w[0].id, "credits");
        assert_eq!(w[0].label, "SuperGrok Heavy");
    ...
    #[test]
    fn sand_usage_becomes_the_bot_window() {
        let v: serde_json::Value = serde_json::from_str(
            r#"{
        ...
        .unwrap();
        let w = parse_bot_usage(&v).expect("bot window");
        assert_eq!(w.id, "bot");
        assert_eq!(w.label, "Grok Bot");
        assert!((w.used - 0.00910429).abs() < 1e-9);
        assert!(w.resets_at.is_some());
    }

    #[test]
    fn sand_usage_without_a_percent_is_absent() {
        let v: serde_json::Value = serde_json::from_str(r#"{ "hasAvailableUsage": false }"#).unwrap();
        assert!(parse_bot_usage(&v).is_none());
    }
}
