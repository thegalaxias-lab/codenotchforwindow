...
/// Cursor often leaves `unfinishedRunAt` set after cancel/abort; `composerData.status` is the
/// authoritative "is a run actually in flight?" signal when present.
fn cursor_run_alive(conn: &rusqlite::Connection, composer_id: &str, unfinished_at: f64, now: u64) -> bool {
    const STALE_RUN_MS: f64 = 15.0 * 60.0 * 1000.0;
    if (now as f64) - unfinished_at > STALE_RUN_MS {
        return false;
    }
    ...
    let Ok(raw) = conn.query_row(
        ...
        // No body row: fall back to the stamp alone (within the stale window)
        return true;
    };
    ...
    match status {
        // Terminal / idle — unfinishedRunAt is stale debris
        "none" | "aborted" | "completed" | "done" | "cancelled" | "canceled" => false,
        // Explicitly working
        "generating" | "preparing" | "streaming" | "running" => true,
        _ => {
            // Unknown status: treat as live only if Cursor still has generating bubbles
            let bubbles = body
                .get("generatingBubbleIds")
                .and_then(|x| x.as_array())
                .map(|a| !a.is_empty())
                .unwrap_or(false);
            bubbles || status.is_empty()
        }
    }
}
...
fn cursor_activity(ctx: &mut Ctx) -> Vec<Activity> {
    ...
    ctx.cursor.refresh(|conn| {
        ...
        for json in rows.flatten() {
            ...
            let Some(id) = v.get("composerId").and_then(|x| x.as_str()) else {
                continue;
            };
            ...
            let state = if blocked {
                "waiting"
            } else if let Some(at) = running {
                if !cursor_run_alive(conn, id, at, now) {
                    continue;
                }
                "busy"
            } else {
...
/// service child (Winsock goes through AFD IOCTLs, which land in the Other counter of the process
/// I/O counters). On Windows that heuristic false-fires constantly, so `claude_activity` is a
/// no-op for now; helpers kept for a future opt-in.
#[allow(dead_code)]
struct IoSample {
    at: u64,
    other: u64,
...
#[allow(dead_code)]
const CLAUDE_RATE_BPS: f64 = 12_000.0; // socket traffic of the network service process only; idle heartbeats stay below this
#[allow(dead_code)]
const CLAUDE_HOLD_MS: u64 = 6_000; // tool calls often leave short gaps; holding briefly avoids flicker without sticking
#[allow(dead_code)]
...
fn claude_activity() -> Vec<Activity> {
    // Network throughput is too noisy on Windows (sync, telemetry, idle sockets) and kept the
    // Claude ring spinning with no live session. Local sessions already go through hooks/watcher.
    vec![]
}
