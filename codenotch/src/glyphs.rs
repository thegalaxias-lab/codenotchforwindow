...
/// Grok Bot launch icon (the rounded robot face), embedded so the ring matches the desktop app.
const GROK_BOT_PNG: &[u8] = include_bytes!("../glyphs/grok-bot.png");

fn builtin_grok_bot() -> Glyph {
    Glyph {
        kind: "appicon".into(),
        url: format!("data:image/png;base64,{}", b64(GROK_BOT_PNG)),
        source: "built-in · Grok Bot launch icon".into(),
        ..Default::default()
    }
}
...
pub fn collect() -> HashMap<String, Glyph> {
    ...
    // Grok Bot gets the launch-icon face (user override still wins via glyph_dirs above).
    if !map.contains_key("grok-bot") {
        // Prefer an override next to the config / exe named grok-bot.png|.svg
        let mut found: Option<Glyph> = None;
        'dirs: for d in &dirs {
            for ext in ["png", "svg"] {
                let p = d.join(format!("grok-bot.{ext}"));
                if let Some(g) = from_file(&p) {
                    found = Some(g);
                    break 'dirs;
                }
        ...
        map.insert(
            "grok-bot".into(),
            found.unwrap_or_else(builtin_grok_bot),
        );
    }
    map
}
