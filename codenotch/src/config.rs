...
pub fn load() -> Config {
    ...
    // Grok Bot is its own ring now; insert it after Grok when an older config only listed Grok.
    migrate_grok_bot_notch(&mut cfg);
...
fn migrate_grok_bot_notch(cfg: &mut Config) {
    if cfg.notch_slots.is_empty() {
        return;
    }
    if cfg.notch_slots.iter().any(|s| s.provider == "grok-bot") {
        return;
    }
    if let Some(i) = cfg.notch_slots.iter().position(|s| s.provider == "grok") {
        cfg.notch_slots
            .insert(i + 1, TraySlot { provider: "grok-bot".into() });
    }
}
