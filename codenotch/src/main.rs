...
pub(crate) fn provider_page(provider: &str) -> Option<(&'static str, &'static str)> {
    Some(match provider {
        ...
        "grok-bot" => ("https://grok.com/?_s=usage", "grok.com"),
...
fn ring_window<'a>(
    ...
    match provider {
        // Prefer a live session window; a 0 % session with no reset (common right after login)
        // falls through to the weekly so the ring shows the number people actually expect.
        "claude" => by_id("session")
            .filter(|w| w.used > 0.0 || w.resets_at.is_some())
            .or_else(|| by_id("weekly_all"))
            .or_else(|| by_id("seven_day"))
            .or_else(|| by_id("weekly"))
            .or_else(|| by_id("session")),
        "codex" => by_id("primary"),
        "cursor" => by_id("included").or_else(|| by_id("auto")).or_else(|| by_id("api")),
        "grok" => by_id("credits").or_else(|| windows.iter().find(|w| w.id != "bot")),
        "grok-bot" => by_id("bot"),
...
pub(crate) fn snapshot_of(app: &AppHandle, id: &str) -> usage::UsageSnapshot {
    ...
    match id {
        ...
        "grok" => {
            let mut s = st.grok.lock().unwrap().clone();
            s.windows.retain(|w| w.id != "bot");
            if s.windows.is_empty() && s.status == "ok" {
                s.status = "absent".into();
            }
            s
        }
        "grok-bot" => {
            let grok = st.grok.lock().unwrap().clone();
            let bot: Vec<_> = grok.windows.iter().filter(|w| w.id == "bot").cloned().collect();
            if bot.is_empty() {
                usage::UsageSnapshot {
                    status: "absent".into(),
                    ..Default::default()
                ...
                usage::UsageSnapshot {
                    status: if grok.status == "absent" || grok.status.is_empty() {
                        "ok".into()
                    } else {
                        grok.status
                    },
                    windows: bot,
                    fetched_at: grok.fetched_at,
                    note: "via Cursor".into(),
                    ..Default::default()
                }
            }
        }
...
pub const TRAY_PROVIDER_IDS: [&str; 7] =
    ["claude", "codex", "glm", "cursor", "grok", "grok-bot", "gemini"];
