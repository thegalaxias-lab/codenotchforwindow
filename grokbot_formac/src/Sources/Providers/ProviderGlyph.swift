...
enum ProviderGlyph: String, Codable, Equatable {
    ...
    case mistral
    case grok
    /// The Grok Bot desktop app's robot face — a different product from the
    /// CLI whose mark `.grok` traces, metered on Cursor's side, so it gets a
    /// mark of its own rather than borrowing the slash.
    ...
    var outline: [[CGPoint]] {
        ...
        // A fallback only: the robot face in the asset catalogue is drawn
        // instead, and the slash stands in when that asset is missing.
        case .grokBot: return GlyphOutline.grok
