...
struct ProviderSnapshot: Identifiable, Equatable {
    ...
    private var authPrompt: String {
        ...
        // The bot's meter rides Cursor's session, so that is the door to knock
        // on — "sign in to Grok Bot" would point at an app with no session of
        // its own to borrow.
        case "grokbot":    return L10n.t("Sign in to Cursor in the editor — Grok Bot's weekly limit rides on that session", locale: locale)
