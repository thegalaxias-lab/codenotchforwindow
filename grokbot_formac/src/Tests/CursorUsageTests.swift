...
final class CursorUsageTests: XCTestCase {
    ...
    /// The dashboard keeps its "Other Models" bar visible at 0%, so the
    /// tooltip does too — a missing row reads as a missing reading.
    func testZeroApiUsageIsStillARow() throws {
        let json = """
        {"individualUsage":{"plan":{"autoPercentUsed":4,"apiPercentUsed":0}}}
        """
        let w = try windows(json)
        XCTAssertEqual(w.map(\.id), ["auto", "api"])
        XCTAssertEqual(w[1].usedFraction ?? -1, 0, accuracy: 0.0001)
        XCTAssertEqual(w[1].label, CursorUsage.otherModelsLabel)
    }
